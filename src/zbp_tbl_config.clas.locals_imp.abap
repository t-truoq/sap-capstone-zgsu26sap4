...
CLASS lhc_tblconfig IMPLEMENTATION.
            ...
            LOOP AT lt_batch_refs INTO DATA(lr_batch_record).
              ...
              IF lv_batch_validation_msg IS NOT INITIAL.
                RAISE EXCEPTION TYPE zcx_excel_pipeline
                  EXPORTING
                    iv_text = |Row { lv_batch_item_no }: { lv_batch_validation_msg }|.
              ENDIF.
              ...
              IF lv_batch_key IS INITIAL.
                RAISE EXCEPTION TYPE zcx_excel_pipeline
                  EXPORTING
                    iv_text = |Cannot build key for CRUD row { lv_batch_item_no }|.
              ENDIF.
        ...
        TRY.
            ...
            LOOP AT lt_batch_refs INTO DATA(lr_batch_record).
              ...
              IF lv_batch_update_validation_msg IS NOT INITIAL.
                RAISE EXCEPTION TYPE zcx_excel_pipeline
                  EXPORTING
                    iv_text = |Row { lv_batch_item_no }: { lv_batch_update_validation_msg }|.
              ENDIF.
        ...
        TRY.
            ...
            LOOP AT lt_batch_refs INTO DATA(lr_batch_record).
              ...
              IF lv_batch_fk_error IS NOT INITIAL.
                RAISE EXCEPTION TYPE zcx_excel_pipeline
                  EXPORTING
                    iv_text = |Row { lv_batch_item_no }: { lv_batch_fk_error }|.
              ENDIF.
  ...
  METHOD gettabledata.
    ...
    LOOP AT lt_config INTO DATA(ls_config).
      ...
      TRY.
          DATA(lv_max_rows) = 1000.
          READ TABLE keys INTO DATA(ls_data_key)
            WITH KEY primary_key COMPONENTS %tky = ls_config-%tky.
          IF sy-subrc = 0 AND ls_data_key-%param-max_rows > 0.
            lv_max_rows = ls_data_key-%param-max_rows.
          ENDIF.
          ...
          DATA lt_pending_records TYPE STANDARD TABLE OF ty_pending_record WITH EMPTY KEY.
          DATA lt_seen_pending_keys TYPE HASHED TABLE OF ztde_record_key WITH UNIQUE KEY table_line.

          SELECT record_key, action_type
            FROM ztbl_aprvl
            WHERE table_name = @ls_config-tablename
              AND status     = 'PENDING'
              AND ( action_type = 'U' OR action_type = 'D' )
            INTO TABLE @DATA(lt_pending_parent).

          LOOP AT lt_pending_parent INTO DATA(ls_pending_parent).
            INSERT ls_pending_parent-record_key INTO TABLE lt_seen_pending_keys.
            IF sy-subrc = 0.
              APPEND VALUE #( record_key  = ls_pending_parent-record_key
                              action_type = ls_pending_parent-action_type ) TO lt_pending_records.
            ENDIF.
          ENDLOOP.

          SELECT item~record_key, item~action_type
            FROM ztbl_aprvl_item AS item
            INNER JOIN ztbl_aprvl AS parent
              ON parent~aprvl_id = item~aprvl_id
            WHERE item~table_name = @ls_config-tablename
              AND item~status     = 'PENDING'
              AND parent~status   = 'PENDING'
              AND ( item~action_type = 'U' OR item~action_type = 'D' )
            INTO TABLE @DATA(lt_pending_items).

          LOOP AT lt_pending_items INTO DATA(ls_pending_item).
            INSERT ls_pending_item-record_key INTO TABLE lt_seen_pending_keys.
            IF sy-subrc = 0.
              APPEND VALUE #( record_key  = ls_pending_item-record_key
                              action_type = ls_pending_item-action_type ) TO lt_pending_records.
            ENDIF.
          ENDLOOP.

          DATA(lv_pending_json) = /ui2/cl_json=>serialize(
            data        = lt_pending_records
            pretty_name = /ui2/cl_json=>pretty_mode-none ).
