CLASS lhc_AuthTablePerm DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.
    METHODS get_global_authorizations FOR GLOBAL AUTHORIZATION
      IMPORTING REQUEST requested_authorizations FOR AuthTablePerm RESULT result.

    METHODS validatetablename FOR VALIDATE ON SAVE
      IMPORTING keys FOR AuthTablePerm~validateTableName.

    METHODS syncuserpermissions FOR DETERMINE ON SAVE
      IMPORTING keys FOR AuthTablePerm~syncUserPermissions.
ENDCLASS.

CLASS lhc_AuthTablePerm IMPLEMENTATION.
  METHOD validatetablename.
    READ ENTITIES OF zi_auth_table_perm IN LOCAL MODE
      ENTITY AuthTablePerm
      FIELDS ( TableName )
      WITH CORRESPONDING #( keys )
      RESULT DATA(lt_permissions).

    LOOP AT lt_permissions INTO DATA(ls_permission).
      SELECT SINGLE active_flag
        FROM ztbl_config
        WHERE table_name = @ls_permission-TableName
        INTO @DATA(lv_active_flag).

      IF sy-subrc <> 0 OR lv_active_flag <> abap_true.
        APPEND VALUE #( %tky = ls_permission-%tky )
          TO failed-authtableperm.
        APPEND VALUE #(
          %tky = ls_permission-%tky
          %msg = new_message_with_text(
            severity = if_abap_behv_message=>severity-error
            text = |Table { ls_permission-TableName } must have an active table configuration| )
          %element = VALUE #( TableName = if_abap_behv=>mk-on )
        ) TO reported-authtableperm.
      ENDIF.
    ENDLOOP.
  ENDMETHOD.

  METHOD get_global_authorizations.
    DATA(lv_auth) = COND #( WHEN zcl_auth_helper=>is_admin( ) = abap_true
                            THEN if_abap_behv=>auth-allowed
                            ELSE if_abap_behv=>auth-unauthorized ).

    result-%create = lv_auth.
    result-%update = lv_auth.
    result-%delete = lv_auth.
  ENDMETHOD.

  METHOD syncuserpermissions.
    READ ENTITIES OF zi_auth_table_perm IN LOCAL MODE
      ENTITY AuthTablePerm
      FIELDS ( TableName CanView CanCreate CanUpdate CanDelete )
      WITH CORRESPONDING #( keys )
      RESULT DATA(lt_policies).

    LOOP AT keys INTO DATA(ls_key).
      READ TABLE lt_policies INTO DATA(ls_policy)
        WITH KEY TableName = ls_key-TableName.

      IF sy-subrc <> 0.
        "A deleted table policy revokes all user-specific permissions.
        DELETE FROM ztbl_user_perm
          WHERE table_name = @ls_key-TableName.
        CONTINUE.
      ENDIF.

      zcl_auth_helper=>apply_table_policy(
        iv_table_name = ls_policy-TableName
        iv_can_view   = ls_policy-CanView
        iv_can_create = ls_policy-CanCreate
        iv_can_update = ls_policy-CanUpdate
        iv_can_delete = ls_policy-CanDelete ).
    ENDLOOP.
  ENDMETHOD.
ENDCLASS.
